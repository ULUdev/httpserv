...
httpserv_http_method_t httpserv_http_strtomethod(const char *s) {
  if (streq(s, "GET"))
    return HTTPSERV_HTTP_METHOD_GET;
  else if (streq(s, "POST"))
    return HTTPSERV_HTTP_METHOD_POST;
  else if (streq(s, "PUT"))
    return HTTPSERV_HTTP_METHOD_PUT;
  else if (streq(s, "DELETE"))
    return HTTPSERV_HTTP_METHOD_DELETE;
  else if (streq(s, "CONNECT"))
    return HTTPSERV_HTTP_METHOD_CONNECT;
  else if (streq(s, "OPTIONS"))
    return HTTPSERV_HTTP_METHOD_OPTIONS;
  else if (streq(s, "TRACE"))
    return HTTPSERV_HTTP_METHOD_TRACE;
  else if (streq(s, "PATCH"))
    return HTTPSERV_HTTP_METHOD_PATCH;
  else
    return HTTPSERV_HTTP_METHOD_INVALID;
}
...
httpserv_http_request_from_raw(httpserv_raw_data_t *raw) {
  ...
  for (int i = pos; i < raw->len; i++) {
    ...
    if (streq(key, "") && streq(value, "") && raw->content[i] == '\r' &&
        raw->content[i + 1] == '\n') {
      break;
    }
    ...
    } else if (keyread && raw->content[i] == '\r' &&
               raw->content[i + 1] == '\n') {
      pos++;
      i++;
      keyread = 0;
...
httpserv_http_request_build(const httpserv_http_request_t *req) {
  ...
  httpserv_raw_data_t *raw = malloc(sizeof(httpserv_raw_data_t));
