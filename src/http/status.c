...
char *httpserv_http_strstatus(const httpserv_http_status_t status) {
  /*
   * This table is from
   * <https://developer.mozilla.org/en-US/docs/Web/HTTP/Status> which should
   * conform to HTTP/1.1. Everything which is marked as experimental or WebDAV
...
int httpserv_http_status_valid(const httpserv_http_status_t status) {
  return httpserv_http_strstatus(status) != NULL;
}
